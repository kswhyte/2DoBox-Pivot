...
const controller = {
  ...
  renderActiveToDosToDOM: function () {
    $('.todo-list').html('')
    let toDosSortedById = toDoBox.activeToDoList.sort(function(a, b) {
      return b.id-a.id;
    })
    for (var i = 0; i < 10; i++) {
  ...
  renderCompletedToDosToDOM: function() {
    if (this.renderCompletedToDos === true) {
      ...
      toDosSortedById.forEach(function(toDo) {
        $('.completed-todos').prepend(toDo.renderToDoToHTML())
      })
      // //this code limits completed ideas to 10
      // let toDosSortedById = toDoBox.completedToDoList.sort(function(a, b) {
      //   return b.id-a.id;
      // })
      // for (var i = 0; i < 10; i++) {
      //   $('.completed-todos').append(toDosSortedById[i].renderToDoToHTML())
      // }
    } else {
      $('.completed-todos').html('')
    }
